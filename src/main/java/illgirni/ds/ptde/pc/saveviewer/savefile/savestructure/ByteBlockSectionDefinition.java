...
/**
 * General definition of a block of bytes in a binary file. The data type of the block's data 
 * is also part of this definition: Once as plain Java type and once as type as defined in the 
 * binary binary file (e.g. long vs. unsigned int 32). 
 * 
 * @author illgirni
 *
...
public class ByteBlockSectionDefinition<T> {
    ...
    /**
     * For a String this should be the number of characters and not the number of bytes!
     * Otherwise this should be the number of bytes in the block. A negative value means 
     * "as much as possible".
     */
     ...
     */
    public ByteBlockSectionDefinition(final int offset, final int length, final JavaTypeToDataType<T> type) {
        this.offset = offset;
        this.length = length;
    ...
    /**
     * Mappings of all the "interesting" types. Introduced for compile time mapping between
     * plain Java type and binary data type ({@link DataType}). 
     * <p/>
     * This is created to try out how to get compile time safety with generics and enums. We 
     * could actually get rid of {@link DataType}.
     * 
     * @author illgirni
     *
    ...
    public static class JavaTypeToDataType<E> {
         ...
         */
        public static JavaTypeToDataType<boolean[]> BITS_OF_BYTE = new JavaTypeToDataType<>(DataType.BITS_OF_BYTE);
         ...
         * Block to block.
         */
        public static JavaTypeToDataType<ByteBlock> BYTE_BLOCK = new JavaTypeToDataType<>(DataType.BYTE_BLOCK);
         ...
         */
        public static JavaTypeToDataType<String> STRING_UTF16 = new JavaTypeToDataType<>(DataType.STRING_UTF16);
