...
public class PreferencesEditController {
  ...
  /**
   * Opens the preference dialog and waits for it to finish. After which the changes to the
    ...
    for (final PreferenceValue newPreference : newPreferences) {
      if (!newPreference.getUseDefault().get()) {
        if (newPreference.getPreference() == PreferenceSetting.FONT_SIZE) {
          ...
          windowPreferences.setProperty(newPreference.getPreference().getPreferencesKey(),
              newPreference.getValue().get() + "");
        }
  ...
  /**
   * Saves the window size and layout preferences. If all settings are the default values deletes
   * the existing preferences file instead.
   * 
  ...
  private void saveFontPreferences(Integer fontSizePreference) {
    ...
    if (fontSizeFile != null) {
      ...
      if (fontSizePreference != null) {
        fontSizeFileContent =
            MessageFormat.format(LayoutUtils.FONT_SIZE_FILE_CONTENT_FORMAT, fontSizePreference);
