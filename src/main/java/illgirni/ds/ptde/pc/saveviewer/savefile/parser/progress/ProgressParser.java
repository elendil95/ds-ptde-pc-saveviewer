...
/**
 * Parses the playthrough's progress from the byte block representing a save slot's content.
 * Mostly splits the byte block in smaller chunks and passes them to the parsers for the 
 * corresponding "topic".
 * 
 * @author illgirni
 *
...
public class ProgressParser extends AbstractSaveElementParser {
    ...
    private BossesProgress parseBossesProgress(final ByteBlock slotContentData) {
        ...
        for (final BossGroupBitBlockDefinition bossGroupDefinition : progressDefinition.getBossGroupDefinitions()) {
            bossGroups.add(new BossGroupBitBlock(blockSectionParser.parse(bossGroupDefinition, slotContentData), 
    ...
    private TailcutsState parseTailcutsState(final ByteBlock slotContentData) {
        ...
        for (final TailcutBitBlockDefinition tailcutDefinition : progressDefinition.getTailcutDefinitions()) {
    ...
    /**
     * Extracts the warp points and warp (un)locked sections from the slot content byte block and hands over to the 
     * corresponding parser to parse them.
     * 
     * @param slotContentData The slot content.
    ...
    private WarpState parseWarpState(final ByteBlock slotContentData) throws ParserException {
        ...
        for (final WarpPointGroupBitBlockDefinition warpPointGroupDefinition : progressDefinition.getWarpPointGroupDefinitions()) {
            warpPointGroups.add(new WarpPointGroupBitBlock(blockSectionParser.parse(warpPointGroupDefinition, slotContentData), 
                                                           warpPointGroupDefinition.getBonfires()));
        }
        
        final ByteBlock warpUnlockedBlock = blockSectionParser.parse(progressDefinition.getWarpUnlockedBlockDefinition(), slotContentData);
        
        return warpStateParser.parse(warpPointGroups, warpUnlockedBlock);
    }
    
    /**
     * Extracts the bonfire kindle states sections from the slot content byte block and hands over to the 
     * corresponding parser to parse them.
     * 
     * @param slotContentData The slot content.
    ...
    private BonfiresState parseBonfiresState(final ByteBlock slotContentData) throws ParserException {
        ...
        final int bonfiresBlockLength = blockSectionParser.parse(progressDefinition.getBonfiresLengthDefinition(bonfiresLengthOffset), slotContentData) 
                                        + getInt32Length();
        
        final ByteBlock bonfiresBlock = blockSectionParser.parse(progressDefinition.getBonfiresDefintion(bonfiresBlockOffset, bonfiresBlockLength), 
                                                                 slotContentData);
    ...
    private int calculateBonfiresLengthOffset(final ByteBlock slotContentData) throws ParserException {
        int bonfiresOffset = progressDefinition.getBonfiresOffsetLengthCalculationDefinition().getOffset();
    ...
    private int calculateNextOffset(final ByteBlock slotContentData, final int offset) throws ParserException {
        return calculateNextOffset(slotContentData, offset, 0);
    }
    
    /**
     * Calculates the offset of the next section after the section starting at the given offset. 
     * The current section is assumed to have a footer of the given length.
     * 
     * @param slotContentData The slot content.
     * @param offset The current offset.
    ...
    private int calculateNextOffset(final ByteBlock slotContentData, final int offset, final int footerLength) throws ParserException {
        final int blockLength = blockSectionParser.parse(progressDefinition.getBonfiresLengthDefinition(offset), slotContentData);
