...
/**
 * Entry point into the application. Sets up all the managers, triggers file loading, and starts the
 * UI.
 * 
 * @author illgirni
 *
...
public class SaveViewer implements UIApplication {
  ...
  /**
   * The directory containing the user settings files. This is actually just the workspace
   * directory.
   * 
  ...
  public File getSettingsDirectory() {
    try {
      ...
      // Discard silently. This only means we haven't set the workspace up yet or user provided
      // non-existing path.
      return null;
    }
  }
  ...
  /**
   * Loads the workspace's save file. Gives the user the option to choose a different save file if
   * the one defined in the workspace cannot be loaded.
   * 
  ...
  private boolean loadSaveFile() throws SaveManagerException {
    ...
    while (!saveFileLoaded) {
      try {
        ...
        BasicDialogs.showWarnDialog("Could not load the save file. Please choose another one.",
            e.getMessage());
        saveFileLoaded = false;
      }
      ...
      if (!saveFileLoaded) {
        final String saveFilePath =
            BasicDialogs.showSaveFilePathDialog(workspace.getSaveFilePath(), "DRAKS0005.sl2");
