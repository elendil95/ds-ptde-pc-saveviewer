...
/**
 * Provides basic dialogs. All dialogs created through this will have the custom application style
 * applied.
 * 
 * @author illgirni
 *
...
public final class BasicDialogs {
  ...
  public static String showSaveFilePathDialog(String saveFilePath, String saveFileName) {
    ...
    final Dialog<String> dialog =
        createDialog("Select DS1-PTDE Save File", "Select Dark Souls - Prepare to Die - Save File");
    ...
    saveFileChooser.getExtensionFilters()
        .add(new ExtensionFilter("DS1 PTDE Save File", saveFileName));

    final Button selectSaveFileButton = new Button("Select...");
    selectSaveFileButton.setOnAction(new EventHandler<ActionEvent>() {
      @Override
      public void handle(final ActionEvent event) {
        final File fileSelection =
            saveFileChooser.showOpenDialog(((Node) event.getSource()).getScene().getWindow());
    ...
    saveFileField.textProperty().addListener(
        (observable, oldValue, newValue) -> okButton.setDisable(newValue.trim().isEmpty()));

    dialog.setResultConverter(
        (button) -> okButtonType.equals(button) ? saveFileField.getText() : null);
  ...
  /**
   * Shows an error dialog. When an exception is provided also shows the stack trace of the
   * exception.
   * 
   * @param errorMessage The error message.
   * @param errorDetails Details about the error.
   ...
   */
  public static Alert createSimpleDialog(final String title, final String headerText,
      final AlertType type, final boolean showOkButton) {
    final Alert dialog;
  ...
  private static <T> void applyDialogIcon(final Dialog<T> dialog) {
    ...
    if (window instanceof Stage) {
      ...
      System.err.println("Cannot set dialog icon. Not a stage: " + window); // FIXME should be
                                                                            // logging.
    }
  }
