...
public class EquipmentPanel extends AbstractDetailsPanel {
    ...
    /**
     * Adds the "list blocks" to the container. The attuned magics block is not added 
     * yet, because the number of attunable magics depends on the character. The nodes 
     * for the attuned magics are created and removed by demand.
     */
    @Override
    ...
    public void setAttunedMagics(final List<AttunedMagic> attunedMagics) {
        ...
        for (int magicIndex = 0; magicIndex < magicGroup1.size(); magicIndex++) {
            ...
            final Pair<TextFlow, Text> attunedMagic = createValueCell(asString(magicGroup1.get(magicIndex)), pos);
        ...
        for (int magicIndex = 0; magicIndex < magicGroup2.size(); magicIndex++) {
            ...
            final Pair<TextFlow, Text> attunedMagic = createValueCell(asString(magicGroup2.get(magicIndex)), pos);
            final Node attunedMagicLabel = createLabelGridCell("Magic " + (magicGroup1.size() + magicIndex + 1), pos);
            magicCells.add(attunedMagicLabel);
            magicCells.add(attunedMagic.getKey());
