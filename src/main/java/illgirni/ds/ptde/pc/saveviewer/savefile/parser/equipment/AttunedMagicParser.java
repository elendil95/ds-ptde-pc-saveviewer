...
public class AttunedMagicParser extends AbstractSaveElementParser {
  ...
  /**
   * Uses the definition to parse a single attuned magic from the byte block. If the attunement slot
   * corresponding to definition is empty this method return {@code null}.
   * 
   ...
   */
  public AttunedMagic parse(final AttunedMagicDefinition attunedMagicDefinition,
      final ByteBlock attunedMagicsBlock) {
    final long itemId =
        blockSectionParser.parse(attunedMagicDefinition.getMagicIdDefinition(), attunedMagicsBlock);
