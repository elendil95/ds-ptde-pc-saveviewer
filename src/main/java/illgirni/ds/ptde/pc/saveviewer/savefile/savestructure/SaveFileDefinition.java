 ...
 * <ol>
 *  <li><i>Magic Number:</i> Magic number indicating that this is a dark souls save file and for which version.</li>
 *  <li><i>Save Slot Definitions:</i> A listing of where to find the save slots (characters) in the save file. This 
 *      makes the save file somewhat self-describing.</li>
 *  <li><i>Save Slot Names:</i> A listing of names of the save slots. The names do not correspond to the character names!</li>
 *  <li><i>Save Slots:</i> A listing of the actual save slots (with character data or empty). The entries in the slot list 
 *      are separated by twelve bytes with value {@code 0}.</li>
 * </ol>
 * Save slots actually always have the same size. The first slot can always be found at the same position. And there is 
...
public class SaveFileDefinition {
    ...
    /**
     * The section in the save file containing the magic number (which are the first 4 bytes of the save file).
     */
    ...
    /**
     * The section with the definitions of the save slots. This method returns only the definition for the first ten slot 
     * definitions/descriptors, because the eleventh is not available in game, which is a waste of about 400 KByte per file...
     */
    public List<ByteBlockSectionDefinition<ByteBlock>> getSlotDescriptorDefinitions() {
        ...
        for (int slotIndex = 0; slotIndex < getNumberOfSaveSlots(); slotIndex++) {
            slotDescriptorDescriptors.add(new ByteBlockSectionDefinition<>(getSlotDescriptorsOffset() + (slotIndex * getSlotDescriptorLength()),
                                                                           getSlotDescriptorLength(), 
     ...
     */
    public ByteBlockSectionDefinition<ByteBlock> getSlotDefinition(final int offset, final int length) {
