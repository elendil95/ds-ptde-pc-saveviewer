...
/**
 * Parses the play time from the byte block. The play time is stored in milliseconds, which are converted 
 * to a time string.
 * 
 * @author illgirni
 *
...
public class PlayTimeParser extends AbstractSaveElementParser {
    ...
    public String parse(ByteBlock playTimeBlock) {
        final long totalMillis = blockSectionParser.parse(playTimeDefinition.getPlayTimeDefinition(), playTimeBlock);
