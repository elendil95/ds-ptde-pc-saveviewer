...
/**
 * Base implementation for a node in a containing groups for exported slots and exported slots.
 * Wraps the group or node that this node represents.
 * 
 * @author illgirni
 *
 * @param <T>
...
public abstract class AbstractTreeNodeWrapper<T> {
  ...
  /**
   * Removes the wrapped children from this wrapper, which are not referenced by this nodes wrapped
   * node anymore.
   * 
  ...
  /**
   * Updates the order of this node's children to correspond to the order of the children of the
   * wrapped node.
   * 
   * @param childrenToWrap The children that the wrapped node currently has.
   */
  private void updateChildOrder(final List<?> childrenToWrap) {
    ...
    if (orderChanged) {
      children.sort((child1, child2) -> childrenToWrap.indexOf(child1.getWrappedNode())
          - childrenToWrap.indexOf(child2.getWrappedNode()));
    }
  }

  /**
   * Gets the index of the child of this node that wraps the given child of this node's wrapped
   * node.
   * 
