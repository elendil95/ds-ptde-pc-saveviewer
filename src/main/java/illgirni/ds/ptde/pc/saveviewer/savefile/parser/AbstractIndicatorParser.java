...
/**
 * Base for parsers that need to parse bytes which represent indicator values (i.e. enumerated
 * values).
 * 
 * @author illgirni
 *
...
public abstract class AbstractIndicatorParser<T> extends AbstractSaveElementParser {
  ...
  public final T parse(final ByteBlock indicatorBlock) throws UnknownIndicatorException {
    final long indicator =
        blockSectionParser.parse(indicatorStatDefinition.getDefinition(), indicatorBlock);
