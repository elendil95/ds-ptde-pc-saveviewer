...
public abstract class AbstractParentGroupSelectionDialog<T> extends Dialog<T> {
   ...
   */
  private final SimpleObjectProperty<TreeItem<AbstractTreeNodeWrapper<?>>> selectedParentTreeNode =
      new SimpleObjectProperty<>();

  /**
   * The currently selected slot group.
   */
  private final TransitiveObjectProperty<TreeItem<AbstractTreeNodeWrapper<?>>, ExportedSlotGroup> selectedParent =
      new TransitiveObjectProperty<>();

  /**
   * Applies the styles to the dialog and links the {@link #getSelectedParent()} to the tree
   * selection.
   */
  public AbstractParentGroupSelectionDialog() {
    BasicDialogs.applySaveViewerStyle(this);

    this.selectedParent.bind(selectedParentTreeNode,
        treeItem -> treeItem.getValue() instanceof ExportedSlotGroupWrapper
   ...
   * @return The tree.
   */
  private TreeView<AbstractTreeNodeWrapper<?>> createParentSelectionTree(
      final ExportedSlotsWrapper wrappedParents) {
  ...
  /**
   * Creates a tree item wrapping the given node. Also creates tree items for the children of the
   * node and adds them as children to the created tree item.
   * 
   * @param treeNodeValue The tree item's value.
   * @return The tree item.
   */
  private TreeItem<AbstractTreeNodeWrapper<?>> createTreeNode(
      final AbstractTreeNodeWrapper<?> treeNodeValue) {
  ...
  /**
   * A tree cell showing the cell's value as name of the value. Also shows an icon in front of the
   * name. Corrects the vertical alignment and click-behavior of the cell's disclosure node.
   *
   */
  ...
  private class ParentTreeCell extends TreeCell<AbstractTreeNodeWrapper<?>> {
    ...
    /**
     * Custom disclosure node implementation that allows to expand or collapse the tree node even
     * when it is currently selected. Also fixes the alignment of the node to be vertically
     * centered.
     */
    private Node createDisclosureNode() {
      ...
      disclosureWrapper
          .setOnMouseClicked(event -> getTreeItem().setExpanded(!getTreeItem().isExpanded()));
  ...
  private class TransitiveObjectProperty<S, R> extends SimpleObjectProperty<R> {
    ...
    /**
     * Gets the value from the observed object by using the selection function when this property is
     * bound to the other observable with {@link #bind(ObservableObjectValue, Function)}. Otherwise
     * behaves like the base implementation.
     */
    @Override
    public R get() {
