...
/**
 * The central controller of the application. Puts the contents in the main window 
 * and handles most of the events fired by the application.
 * 
 * @author illgirni
 *
 */
public class MainController {
    ...
    /**
     * The workspace manager through which all changes to the work space and save file
     * have to be directed.
     */
    ...
    /**
     * Shows the main window. Before showing it though, it applies the
     * user preference values to the main window.
     */
    public void show() {
    ...
    /**
     * Initializes the main window and sets the window contents (retrieved
     * from the sub-controllers). Also registers the menu event handler at the 
     * main window.
     */
    ...
    /**
     * Initializes the sub-controller for the display of the slots in the save file.
     * Registers the save file (slot) related event handlers at the panel managed
    ...
    private void initSaveFileSlotsController() throws SaveManagerException {
        if (saveFileSlotsController == null) {
            saveFileSlotsController = new SaveFileSlotsController(workspaceManager.getWorkspace().getSaveFile());
            ...
            slotsPanel.addEventHandler(ShowSaveFileSlotEvent.SHOW_SAVE_FILE_SLOT, new ShowSaveFileSlotHandler());
            
        }
    }
    
    /**
     * Initializes the sub-controller for the display of the exported slots. Registers the 
     * exported slot (group) related event handlers at the panel managed by the sub-controller.
     * 
     * @throws SaveManagerException
     */
    private void initExportedSlotsController() throws SaveManagerException {
        if (exportedSlotsController == null) {
            exportedSlotsController = new ExportedSlotsController(workspaceManager.getWorkspace().getExportedSlots());
            ...
            slotsPanel.addEventHandler(RenameGroupEvent.RENAME_GROUP, new RenameExportedSlotGroupHandler());
            slotsPanel.addEventHandler(RenameSlotEvent.RENAME_SLOT, new RenameExportedSlotHandler());
            
            slotsPanel.addEventHandler(DeleteGroupEvent.DELETE_GROUP, new DeleteExportedSlotGroupHandler());
            ...
            slotsPanel.addEventHandler(ShowExportedSlotEvent.SHOW_EXPORTED_SLOT, new ShowExportedSlotHandler());
            
        }
    }
    
    /**
     * Initializes the sub-controller for the display of the details of a selected save slot. Registers the 
     * event handlers for importing and exporting slots at the panel managed by the sub-controller.
     */
    ...
    /**
     * EventHandler to reload the save file in the workspace. Also triggers the corresponding
     * refresh in the save file slot listing and save slot details display.
     */
    private class ReloadHandler implements EventHandler<ReloadSaveEvent> {
        ...
        public void handle(final ReloadSaveEvent event) {
            try {
                ...
                BasicDialogs.showErrorDialog("An error occurred while reloading the save file.", e.getMessage(), e);
            }
        }
    ...
    private class ExportSlotHandler implements EventHandler<ExportSlotEvent> {
        ...
        public void handle(final ExportSlotEvent event) {
            try {
                ...
                final ExportSlotDialog exportDialog = new ExportSlotDialog(characterName, workspaceManager.getWorkspace().getExportedSlots());
                
                final Optional<Pair<String, ExportedSlotGroup>> exportedSlotData = exportDialog.showAndWait();
                
                if (exportedSlotData.isPresent()) {
                    workspaceManager.exportSlot(slotToExport, exportedSlotData.get().getKey(), null, exportedSlotData.get().getValue());
                    exportedSlotsController.refreshSlotTree();
                    
                } 
                
            } catch (SaveManagerException e) {
                BasicDialogs.showErrorDialog("An error occurred while trying to export slot.", e.getMessage(), e);
            }
        }
     ...
     * position at which to put the slot in the save file. When the dialog is confirmed actually imports 
     * the slot and triggers the required UI refreshes.
     */
     ...
     * parent group and define the group name. When the dialog is confirmed actually creates the group and 
     * triggers the required UI refreshes.
     */
        ...
        public void handle(final AddGroupEvent event) {
            try {
                ...
                if (newGroupValues.isPresent()) {
                    workspaceManager.createSlotGroup(newGroupValues.get().getKey(), newGroupValues.get().getValue());
                    exportedSlotsController.refreshSlotTree();
                    
                }
                
            } catch (SaveManagerException e) {
                BasicDialogs.showErrorDialog("An error occurred while trying to add group.", e.getMessage(), e);
            }
    ...
    /**
     * Handler managing the renaming of a group of exported slots. Shows the dialog to  define the 
     * new group name. When the dialog is confirmed actually renames the group and triggers the 
     * required UI refreshes.
     */
    private class RenameExportedSlotGroupHandler implements EventHandler<RenameGroupEvent> {
        ...
        public void handle(final RenameGroupEvent event) {
            try {
                ...
                final RenameExportedElementDialog renameDialog = new RenameExportedElementDialog(renamedGroup.getName());
                ...
                BasicDialogs.showErrorDialog("An error occurred while trying to renaming group.", e.getMessage(), e);
            }
        }
    ...
    /**
     * Handler managing the renaming of an exported slot. Shows the dialog to define the  new 
     * slot name. When the dialog is confirmed actually renames the slot and triggers the 
     * required UI refreshes.
     */
    private class RenameExportedSlotHandler implements EventHandler<RenameSlotEvent> {
        ...
        public void handle(final RenameSlotEvent event) {
            try {
                ...
                final RenameExportedElementDialog renameDialog = new RenameExportedElementDialog(renamedSlot.getName());
                ...
                BasicDialogs.showErrorDialog("An error occurred while trying to renaming slot.", e.getMessage(), e);
            }
        }
    ...
    /**
     * Handler managing the deletion of an exported slot group. Shows a confirmation dialog before actually
     * deleting the group and its contents. Triggers the required UI refreshes afterwards.
     */
    private class DeleteExportedSlotGroupHandler implements EventHandler<DeleteGroupEvent> {
        ...
        public void handle(final DeleteGroupEvent event) {
            try {
                ...
                BasicDialogs.showErrorDialog("An error occurred while trying to delete group.", e.getMessage(), e);
            }
        }
    ...
    private class DeleteExportedSlotHandler implements EventHandler<DeleteSlotEvent> {
        ...
        public void handle(final DeleteSlotEvent event) {
            try {
                ...
                BasicDialogs.showErrorDialog("An error occurred while trying to delete character.", e.getMessage(), e);
            }
        }
    ...
    /**
     * Handler managing moving an exported slot to a different group. Shows the move dialog in which the new group
     * has to be chosen. When the dialog is confirmed actually moves the slot and triggers the required UI updates.
     */
    private class MoveExportedSlotHandler implements EventHandler<MoveSlotEvent> {
        ...
        public void handle(final MoveSlotEvent event) {
            try {
                ...
                final ExportedSlotsWrapper selectableParentGroups = new ExportedSlotsWrapper(workspaceManager.getWorkspace().getExportedSlots(), true);
                final AbstractMoveExportedElementDialog moveDialog = new MoveExportedSlotDialog(movedSlot.getName(), selectableParentGroups);
                ...
                BasicDialogs.showErrorDialog("An error occurred while trying to move slot.", e.getMessage(), e);
            }
        }
    ...
    /**
     * Handler managing moving an exported slot group to a different parent group. Shows the move dialog in 
     * which the new parent group has to be chosen. When the dialog is confirmed actually moves the group 
     * and triggers the required UI updates.
     */
    private class MoveExportedSlotGroupHandler implements EventHandler<MoveGroupEvent> {
        ...
        public void handle(final MoveGroupEvent event) {
            try {
                ...
                final AbstractMoveExportedElementDialog moveDialog = new MoveExportedSlotGroupDialog(movedGroup.getName(), selectableParentGroups);
                ...
                BasicDialogs.showErrorDialog("An error occurred while trying to move slot group.", e.getMessage(), e);
            }
        }
        
        /**
         * Removes the group to move from from the parents selectable in the move dialog. This avoids cycles and self 
         * references. 
         * 
         ...
         */
        private boolean removeFromParentWrapper(final ExportedSlotGroup movedGroup, final AbstractTreeNodeWrapper<?> parentCandidate) {
            boolean removed = parentCandidate.getChildren().removeIf(group -> Objects.equals(movedGroup, group.getWrappedNode()));
    ...
    /**
     * Handler to manage showing the details of a slot in the save file. Also clears the selection
     * in the exported slots panel.
     */
    private class ShowSaveFileSlotHandler implements EventHandler<ShowSaveFileSlotEvent> {
        ...
        public void handle(ShowSaveFileSlotEvent event) {
            //clear selection first and update slots controller second (because selection clear may trigger another listener)
            ...
            try {
                if (newSlot != null) {
                    if (newSlot.getSlot() == null) {
                        //"lazy loading" the slot data. we saved application startup time by not loading that earlier.
    ...
    private class MenuHandler implements EventHandler<MenuEvent> {
        ...
        public void handle(MenuEvent event) {
            try {
                ...
                BasicDialogs.showErrorDialog("Uuups...", "This should not have happened: " + e.getMessage(), e);
            }
        }
