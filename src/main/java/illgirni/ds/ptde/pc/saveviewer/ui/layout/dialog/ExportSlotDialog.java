...
/**
 * The dialog to choose the exported slot group in which to put a newly exported slot
 * and the custom name for the exported slot.
 * <p/>
 * The result of the dialog is a pair consisting of name of the exported slot and the 
 * group that shall contain the slot.
 * 
 * @author illgirni
 *
 */
public class ExportSlotDialog extends AbstractParentGroupSelectionDialog<Pair<String, ExportedSlotGroup>> {
    ...
    public ExportSlotDialog(final String characterName, final ExportedSlots parentGroups) {
        ...
        name.addListener((observable, oldValue, newValue) -> defineOkButtonState(okButton, newValue, getSelectedParent().get()));
        getSelectedParent().addListener((observable, oldValue, newValue) -> defineOkButtonState(okButton, name.get(), newValue));
        
        setResultConverter(pressedButton -> SAVE_BUTTON_TYPE.equals(pressedButton) ? new Pair<>(name.get(), getSelectedParent().get()) : null);
        
    }
    
    private void defineOkButtonState(final Node okButton, final String name, final ExportedSlotGroup group) {
