...
/**
 * The dialog in which to choose the position at which to import an exported slot into the save
 * file.
 * <p/>
...
public class ImportSlotDialog extends Dialog<Integer> {
  ...
  public ImportSlotDialog(final String characterName, final List<SaveSlot> replacableSlots) {
    ...
    setHeaderText(
        "Import character '" + characterName + "' into save file. Select replaced character.");
    ...
    replacedSlotField.getSelectionModel().selectedItemProperty()
    ...
    setResultConverter(
        pressedButton -> SAVE_BUTTON_TYPE.equals(pressedButton) ? selectedSlot.get() : null);
    ...
    protected void updateItem(SaveSlot item, boolean empty) {
      ...
      if (empty) {
        ...
        if (item.isEmpty()) {
          ...
          label.setText(
              displayedIndex + ". " + item.getContent().getCharacterStatistics().getName());
        }
