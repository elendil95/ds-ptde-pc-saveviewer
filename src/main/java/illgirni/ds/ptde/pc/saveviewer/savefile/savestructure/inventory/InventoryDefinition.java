...
/**
 * The inventory section in the slot content starts always starts at the same offset and always has
 * the same size. The items in the inventory are all defined with the same number of bytes. For each
 * slot in the inventory an item is defined. The values of the item define, if the inventory slot is
 * empty or not.
 * <p/>
 * Also, the items in the inventory are simply listed. There are no particular sections for
 * consumables, weapons, magics, etc.
 * 
 * @author illgirni
 *
 */
...
public class InventoryDefinition {
  ...
  public List<ByteBlockSectionDefinition<ByteBlock>> getInventoryItemDefinitions() {
    ...
    for (int itemIndex = 0; itemIndex < ITEM_SLOTS; itemIndex++) {
      itemDefinitions.add(new ByteBlockSectionDefinition<>(itemIndex * ITEM_SLOT_LENGTH,
          ITEM_SLOT_LENGTH, JavaTypeToDataType.BYTE_BLOCK));
    }
