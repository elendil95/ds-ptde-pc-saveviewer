 ...
 * order of the warp points corresponds to the discovery order in a "usual playthrough".
 * <p/>
...
public class WarpPanel extends AbstractDetailsPanel {
  ...
  private int createBlock1(final GridPane warpPointsContainer, final int column, int row) {
    List<Bonfire> warpPointsBlock = Arrays.asList(Bonfire.FIRELINK_SHRINE, Bonfire.SUNLIGHT_ALTAR,
        Bonfire.UNDEAD_PARISH, Bonfire.DEPTHS, Bonfire.DAUGHTER_OF_CHAOS);
  ...
  private int createBlock2(GridPane warpPointsContainer, int column, int row) {
    List<Bonfire> warpPointsBlock = Arrays.asList(Bonfire.STONE_DRAGON, Bonfire.ANOR_LONDO,
  ...
  private int createBlock3(GridPane warpPointsContainer, int column, int row) {
    List<Bonfire> warpPointsBlock = Arrays.asList(Bonfire.DUKES_ARCHIVE, Bonfire.CRYSTAL_CAVES,
        Bonfire.TOMB_OF_GIANTS_1, Bonfire.GRAVELORD_ALTAR, Bonfire.ABYSS);
  ...
  private int createBlock4(GridPane warpPointsContainer, int column, int row) {
    List<Bonfire> warpPointsBlock =
        Arrays.asList(Bonfire.SANCTUARY_GARDEN, Bonfire.OOLACILE_SANCTUARY,
   ...
   */
  private void createBlock(GridPane warpPointsContainer, int column, int row,
      List<Bonfire> warpPointsBlock) {
    ...
    for (int warpPointIndex = 0; warpPointIndex < warpPointsBlock.size(); warpPointIndex++) {
      ...
      addLabelGridCell(warpPointsContainer, getWarpPointName(warpPoint), position, labelColumn,
          row);
      warpPoints.put(warpPoint,
          addValueCell(warpPointsContainer, "", position, valueColumn, row++));
