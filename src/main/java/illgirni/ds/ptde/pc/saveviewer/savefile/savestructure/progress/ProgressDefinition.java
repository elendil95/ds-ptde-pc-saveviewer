...
/**
 * This is the overall definition of the current playthrough's progress in the slot content
 * of a save slot. The progress definition does not include everything the game has to offer, 
 * but the some important progress indicators. These are:
 * <ul>
...
public class ProgressDefinition {
    ...
    /**
     * The section with the bonfire kindle information is one of a list of sections with dynamically growing 
     * size. This means that its offset in the slot content shifts around. This value here is the offset
     * of the first section in the list, which does not move around. For hints regarding the calculation 
     * of the bonfire kindle state section offset and length see {@link #getBonfiresDefintion(int, int)}.
     */
    ...
    /**
     * Definition of the bonfire kindle state section length. Essentially the first four bytes 
     * in that section. Well, the length definition is not part of the actual section.  
     * 
    ...
    /**
     * Defines the section in the slot content, which contains the bonfire kindle states. This
     * section is one of a number of sections in a list of section that grow as the game 
     * progresses. It itself also grows: The size depends on the locations that have been loaded 
     ...
     *      previous growing sections in that they have a 4 byte footer, which is not part of their 
     *      length definition. So, iterate six times, each time reading the length, skipping that number 
     *      of bytes, and skipping the footer. Just for information: Some of these section contain 
     *      online play information.</li>
     *  <li>With the last calculated offset we have the offset of the bonfire kindle state section in 
     *      the slot content. The bonfire section starts right after this length definition. So, read 
     *      the length and then we have the offset and length of the bonfire section.</li>
     * </ol>
     * Visually this looks something like that:
     * <code><pre>
     * ----------------------------------------------
     ...
     * </pre></code>
     * 
     * @param offset The offset of the bonfire section in the slot content (not including 
     *               the length definition of the section).
    ...
    public ByteBlockSectionDefinition<ByteBlock> getWarpUnlockedBlockDefinition() {
        return new ByteBlockSectionDefinition<>(151597, 1, JavaTypeToDataType.BYTE_BLOCK); // use 127360 to enable warping in game (000000x0)
    }
    
    /**
     * The sections in the slot content, which contain information about which warp points are unlocked.
     */
    public List<WarpPointGroupBitBlockDefinition> getWarpPointGroupDefinitions() {
        ...
        warpPointGroups.add(new WarpPointGroupBitBlockDefinition(127293, 
                                                                 Bonfire.PAINTED_WORLD,
                                                                 Bonfire.TOMB_OF_GIANTS_1,
                                                                 Bonfire.DUKES_ARCHIVE,
                                                                 Bonfire.CRYSTAL_CAVES));
        
        warpPointGroups.add(new WarpPointGroupBitBlockDefinition(127294, 
                                                                 Bonfire.DARKMOON_TOMB,
                                                                 Bonfire.SANCTUARY_GARDEN,
        ...
        warpPointGroups.add(new WarpPointGroupBitBlockDefinition(127295, 
                                                                 Bonfire.FIRELINK_SHRINE,
                                                                 Bonfire.STONE_DRAGON,
    ...
    /**
     * The sections in the slot content, which contain information about which bosses have been defeated.
     */
    public List<BossGroupBitBlockDefinition> getBossGroupDefinitions() {
        List<BossGroupBitBlockDefinition> bossGroups = new ArrayList<>();
        
        bossGroups.add(new BossGroupBitBlockDefinition(127270, 
                                                       Boss.ASYLUM_DEMON, 
                                                       Boss.MANUS));
        
        bossGroups.add(new BossGroupBitBlockDefinition(127271, 
                                                       Boss.QUELAAG, 
                                                       Boss.BED_OF_CHAOS, 
                                                       Boss.IRON_GOLEM, 
                                                       Boss.ORNSTEIN_AND_SMOUGH, 
                                                       Boss.FOUR_KINGS, 
                                                       Boss.SEATH, 
                                                       Boss.GWYN));
        
        bossGroups.add(new BossGroupBitBlockDefinition(127272, 
                                                       Boss.GAPING_DRAGON, 
                                                       Boss.BELL_GARGOYLES, 
                                                       Boss.PRISCILLA, 
                                                       Boss.SIF, 
        ...
        bossGroups.add(new BossGroupBitBlockDefinition(131224, 
                                                       Boss.TAURUS, 
                                                       Boss.CAPRA_DEMON));
        
        bossGroups.add(new BossGroupBitBlockDefinition(135064, 
                                                       Boss.MOONLIGHT_BUTTERFLY));
        
        bossGroups.add(new BossGroupBitBlockDefinition(136232, 
                                                       Boss.SANCTUARY_GUARDIAN, 
                                                       Boss.ARTORIAS, 
                                                       Boss.KALAMEET));
        
        bossGroups.add(new BossGroupBitBlockDefinition(142677, 
                                                       Boss.DEMON_FIRESAGE));
        
        bossGroups.add(new BossGroupBitBlockDefinition(142744, 
                                                       Boss.CEASELESS_DISCHARGE, 
                                                       Boss.CENTIPEDE_DEMON));
        ...
        // -- 145324 (0000x010)? 
        // -- 151809 (x0000000)
        bossGroups.add(new BossGroupBitBlockDefinition(145207, 
                                                       Boss.GWYNDLOLIN)); 
        
        bossGroups.add(new BossGroupBitBlockDefinition(150424, 
                                                       Boss.STRAY_DEMON));
