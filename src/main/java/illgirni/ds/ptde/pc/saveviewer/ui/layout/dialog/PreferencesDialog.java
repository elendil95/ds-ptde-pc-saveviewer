...
/**
 * The menu dialog, which allows to change the user preferences. The result of the dialog
 * is the list of new preference values.
 * 
 * @author illgirni
 *
 */
public class PreferencesDialog extends Dialog<List<PreferenceValue>> {
    ...
    public PreferencesDialog() {
        ...
        final Text restartLabel = new Text("Restart the application after saving to make changes take effect everywhere!");
        ...
        final PreferenceValue characterDetailsValue = new PreferenceValue(PreferenceSetting.CHARACTER_DETAILS);
        ...
        final PreferenceValue windowMaximizedValue = new PreferenceValue(PreferenceSetting.WINDOW_MAXIMIZED);
        ...
        final PreferenceValue windowDividerValue = new PreferenceValue(PreferenceSetting.WINDOW_DIVIDER);
    ...
    /**
     * Creates the nodes for the character details layout preference. This are a label,
     * a "tabs" radio button, a "split" radio button, and a "list radio button.
     * 
    ...
    private List<Node> createCharacterDetailsPreferenceRow(final PreferenceValue preferenceValue) {
        ...
        tabsToggle.setSelected(preferenceValue.getUseDefault().get() || preferenceValue.getValue().get() == 1);
        tabsToggle.selectedProperty().addListener((toggle, oldSel, newSel) -> {if (newSel) {preferenceValue.getValue().set(1);}});
        preferenceValue.getUseDefault().bind(tabsToggle.selectedProperty());
        
        final ToggleButton splitToggle = new RadioButton("Split");
        splitToggle.setSelected(!preferenceValue.getUseDefault().get() && preferenceValue.getValue().get() == 2);
        splitToggle.selectedProperty().addListener((toggle, oldSel, newSel) -> {if (newSel) {preferenceValue.getValue().set(2);}});
        
        final ToggleButton listToggle = new RadioButton("List");
        listToggle.setSelected(!preferenceValue.getUseDefault().get() && preferenceValue.getValue().get() == 3);
        listToggle.selectedProperty().addListener((toggle, oldSel, newSel) -> {if (newSel) {preferenceValue.getValue().set(3);}});
    ...
    private List<Node> createFullscreenPreferenceRow(final PreferenceValue preferenceValue) {
        ...
        noToggle.setSelected(preferenceValue.getUseDefault().get() || preferenceValue.getValue().get() != 1);
        noToggle.selectedProperty().addListener((toggle, oldSel, newSel) -> {if (newSel) {preferenceValue.getValue().set(0);}});
        preferenceValue.getUseDefault().bind(noToggle.selectedProperty());
        
        final ToggleButton yesToggle = new RadioButton("Yes");
        yesToggle.setSelected(!preferenceValue.getUseDefault().get() && preferenceValue.getValue().get() == 1);
        yesToggle.selectedProperty().addListener((toggle, oldSel, newSel) -> {if (newSel) {preferenceValue.getValue().set(1);}});
     ...
     */
    private List<Node> createPreferenceValueRow(final PreferenceValue preferenceValue, final String labelText) {
        final ToggleGroup defaultCustomToggles = new ToggleGroup();
        ...
        final ToggleButton defaultToggle = new RadioButton("Default (" + preferenceValue.getPreference().getDefaultValue() + ")");
        ...
        customValueSlider.valueProperty().addListener((obs, oldVal, newVal) -> {preferenceValue.getValue().set(newVal.intValue());
                                                                                customValueSlider.setValue(preferenceValue.getValue().get());});
        ...
        final TextField customValueOutput = new TextField(valueFormat.format(preferenceValue.getValue().get()));
        customValueOutput.setDisable(true);
        customValueOutput.setAlignment(Pos.CENTER_RIGHT);
        preferenceValue.getValue().addListener((observable, oldValue, newValue) -> customValueOutput.setText(valueFormat.format(newValue)));
        ...
        if (preferenceValue.getPreference() == PreferenceSetting.FONT_SIZE) {
            Platform.runLater(() -> {customValueSlider.setValue(preferenceLabel.getFont().getSize());});
            ...
            customValueSlider.setMajorTickUnit((customValueSlider.getMax() - customValueSlider.getMin()) / 10);
            customValueSlider.setMinorTickCount(3);
            customValueSlider.setBlockIncrement(10);
            
        }
        
        return Arrays.asList(preferenceLabel, defaultToggle, customToggle, customValueSlider, customValueOutput);
    }
    
    /**
     * Creates a copy of the preference values shown i the dialog at the timethe save button is clicked.
     */
