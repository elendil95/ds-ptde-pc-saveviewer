 ...
 * shown as a tree-table. The panel also has controls to modify the tree. It fires an event every time 
 * an exported slot has been selected.
 * 
 * @author illgirni
 *
 */
public class ExportedSlotsPanel extends BorderPane {
     ...
     */
    private final SimpleObjectProperty<ExportedSlotWrapper> selectedSlotProperty = new SimpleObjectProperty<>();
    ...
    public ExportedSlotsPanel() {
        ...
        titleBar.setStyle("-fx-background-color: #fff; -fx-border-width: 0 0 1 0; -fx-border-color: #999;");
        setTop(titleBar);
    ...
    /**
     * Sets the contents of the tree-table to the given tree data. Wraps
     * the tree nodes in {@link TreeItems TreeItem}.
     * 
    ...
    private TreeTableView<AbstractTreeNodeWrapper<?>> setupSlotTree() {
        ...
        slotTree.setOnKeyPressed(keyEvent -> {if (keyEvent.getCode() == KeyCode.ESCAPE) {clearSelection();}});
        
        final TreeTableViewSelectionModel<AbstractTreeNodeWrapper<?>> treeSelectionModel = slotTree.getSelectionModel();
     ...
     */
    private TreeItem<AbstractTreeNodeWrapper<?>> createTreeItem(final AbstractTreeNodeWrapper<?> node) {
        final TreeItem<AbstractTreeNodeWrapper<?>> treeNode = new TreeItem<AbstractTreeNodeWrapper<?>>(node);
    ...
    private void createColumns(TreeTableView<AbstractTreeNodeWrapper<?>> slotTree) {
        ...
        nameColumn.prefWidthProperty().bind(slotTree.widthProperty()
                                                    .subtract(Bindings.when(deleteColumn.visibleProperty()).then(deleteColumn.widthProperty()).otherwise(0))
                                                    .subtract(Bindings.when(moveColumn.visibleProperty()).then(moveColumn.widthProperty()).otherwise(0))
                                                    .subtract(editColumn.widthProperty())
                                                    .subtract(25));
    ...
    private class TreeItemChildrenChangedListener implements ListChangeListener<AbstractTreeNodeWrapper<?>> {
        /**
        ...
        public void onChanged(Change<? extends AbstractTreeNodeWrapper<?>> change) {
            final ObservableList<TreeItem<AbstractTreeNodeWrapper<?>>> treeItemChildren = treeNode.getChildren();
            final ObservableList<? extends AbstractTreeNodeWrapper<?>> changedList = change.getList();
            
            while (change.next()) {
                if (!change.wasUpdated()) {
                    if (change.wasRemoved()) {
                        //Clear tree selection if we remove something. DoesnÄt matter, if this node was affected.
                        clearSelection(); 
                        
                        final List<? extends AbstractTreeNodeWrapper<?>> removedNodes = change.getRemoved();
                        treeItemChildren.removeIf(treeItemChild -> removedNodes.contains(treeItemChild.getValue()));
                    }
                    ...
                    if (change.wasPermutated()) {
                        ...
                        treeItemChildren.sort((treeItemChild1, treeItemChild2) 
                                                    -> changedList.indexOf(treeItemChild1.getValue()) - changedList.indexOf(treeItemChild2.getValue()));
    ...
    /**
     * Custom listener for tree-table node selections. When the newly selected node is an exported slot
     * and the exported slot does not correspond to the last selected one 
     * ({@link ExportedSlotsPanel#selectedSlotProperty}) fires an "selected slot changed" event. In case of 
     * de-selection always fires the event.
     */
    private class TreeSelectionListener implements ChangeListener<TreeItem<AbstractTreeNodeWrapper<?>>> {
        @Override
        public void changed(final ObservableValue<? extends TreeItem<AbstractTreeNodeWrapper<?>>> observable,
            ...
            if (newSelection != null) {
                if (newSelection.getValue() instanceof ExportedSlotWrapper) {
                    ...
                    if (!Objects.equals(newSelectedSlot, selectedSlotProperty.get())) {
                        ...
                        exportedSlotsTree.fireEvent(new ShowExportedSlotEvent(newSelectedSlot.getWrappedNode()));
                    }
                } 
    ...
    /**
     * Factory for the values shown in the name column of the tree-table. The value is a node consisting of an icon
     * and the cell value's name.
     */
    private class NameColumnCellValueFactory implements Callback<CellDataFeatures<AbstractTreeNodeWrapper<?>, Node>, ObservableValue<Node>> {
        @Override
    ...
    /**
     * Factory for the values shown in the edit name column of the tree-table. The value is an icon button which fires a rename 
     * event matching the node's value.
     */
    private class EditColumnCellValueFactory implements Callback<CellDataFeatures<AbstractTreeNodeWrapper<?>,Node>, ObservableValue<Node>> {
        @Override
        public ObservableValue<Node> call(CellDataFeatures<AbstractTreeNodeWrapper<?>, Node> cellData) {
    ...
    /**
     * Factory for the values shown in the move node column of the tree-table. The value is an icon button which fires a move 
     * event matching the node's value.
     */
    private class MoveColumnCellValueFactory implements Callback<CellDataFeatures<AbstractTreeNodeWrapper<?>,Node>, ObservableValue<Node>> {
        @Override
        public ObservableValue<Node> call(CellDataFeatures<AbstractTreeNodeWrapper<?>, Node> cellData) {
            final TreeItem<AbstractTreeNodeWrapper<?>> treeItem = cellData.getValue();
            final AbstractTreeNodeWrapper<?> treeItemValue = treeItem.getValue();
    ...
    /**
     * Factory for the values shown in the delete node column of the tree-table. The value is an icon button which fires a delete 
     * event matching the node's value.
     */
    private class DeleteColumnCellValueFactory implements Callback<CellDataFeatures<AbstractTreeNodeWrapper<?>,Node>, ObservableValue<Node>> {
        @Override
        public ObservableValue<Node> call(CellDataFeatures<AbstractTreeNodeWrapper<?>, Node> cellData) {
            ...
            if (treeItemValue instanceof ExportedSlotGroupWrapper) {
                ...
                deleteButton.setOnAction(event -> deleteButton.fireEvent(new DeleteGroupEvent(deletedGroup)));
     ...
     * disclosure node is not clickable (i.e. the children of the node expandable or collapsible)
    ...
    private class TreeTableViewRowFactory implements Callback<TreeTableView<AbstractTreeNodeWrapper<?>>, TreeTableRow<AbstractTreeNodeWrapper<?>>> {

        @Override
        public TreeTableRow<AbstractTreeNodeWrapper<?>> call(TreeTableView<AbstractTreeNodeWrapper<?>> param) {
            ...
            disclosureWrapper.setOnMouseClicked(event -> row.getTreeItem().setExpanded(!row.getTreeItem().isExpanded()));
