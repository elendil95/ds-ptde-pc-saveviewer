...
/**
 * Parses the bonfire kindle states from the byte block with the information of the already
 * potentially accessible bonfires in the game.
 * 
 * @author illgirni
 *
 */
...
public class BonfiresStateParser extends AbstractSaveElementParser {
  ...
  public BonfiresState parse(final ByteBlock bonfiresBlock) throws ParserException {
    ...
    for (int offset = 0; offset + bonfireStateBlockLength < bonfiresBlock.getLength(); offset +=
        bonfireStateBlockLength) {
      final ByteBlock bonfireStateBlock = blockSectionParser
          .parse(bonfireStatesDefinition.getBonfireStateDefinition(offset), bonfiresBlock);
