...
/**
 * A panel listing the slots of the save file. Non-empty, non-erroneous slots are made
 * selectable. In that case proper events are fired when the slot is selected.
 * 
 * @author illgirni
 *
 */
    ...
    public void setSlots(final List<SaveSlot> slots) {
        ...
        for (final SaveSlot slot : slots) {
            ...
            if (!slot.isEmpty() && !slot.isErroneous()) {
                slotPanel.setOnMouseClicked(event -> { clearSelection(); 
