 ...
 * <p/>
 * The result is a pair consisting of the name of the new group and the parent of the new group.
 * When the parent in the result is {@code null}, this indicates that the new group is a root group.
 * 
 * 
 * @author illgirni
 *
 */
public class AddExportedSlotGroupDialog
  ...
  public AddExportedSlotGroupDialog(final ExportedSlots parentGroups) {
    ...
    groupName.addListener((observable, oldValue, newValue) -> okButton
        .setDisable(newValue == null || newValue.trim().isEmpty()));

    setResultConverter(pressedButton -> SAVE_BUTTON_TYPE.equals(pressedButton)
        ? new Pair<>(groupName.get(), getSelectedParent().get())
        : null);
