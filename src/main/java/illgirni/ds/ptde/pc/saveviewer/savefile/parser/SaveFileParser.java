...
/**
 * Uses the {@link SaveFileDefinition} to parse a save file. Delegates to sub-parsers for the save
 * slot parsing. Validates that the parsed file is indeed an "understandable" file byte checking the
...
public class SaveFileParser extends AbstractSaveElementParser {
  ...
  public SaveFile parse(final String filePath) throws IOException {
    ...
    if (file.isFile()) {
      ...
      if (isDs1SaveFile(saveFileData)) {
        for (int slotIndex = 0; slotIndex < saveFileDefinition.getSlotDescriptorDefinitions()
            .size(); slotIndex++) {
          final ByteBlockSectionDefinition<ByteBlock> slotDescriptorDefinition =
              saveFileDefinition.getSlotDescriptorDefinitions().get(slotIndex);
          final ByteBlock slotDescriptorData =
              blockSectionParser.parse(slotDescriptorDefinition, saveFileData);
          final SaveSlotDescriptor slotDescriptor =
              slotDescriptorParser.parse(slotDescriptorData, slotIndex);

          final String slotName = blockSectionParser.parse(
              saveFileDefinition.getSlotNameDefinition(slotDescriptor.getSlotNameOffset()),
              saveFileData);

          final ByteBlock slotData = blockSectionParser.parse(saveFileDefinition.getSlotDefinition(
              slotDescriptor.getSlotOffset(), slotDescriptor.getSlotLength()), saveFileData);

          final ByteBlock loadScreenData = blockSectionParser
              .parse(saveFileDefinition.getSlotLoadScreenDefinition(slotIndex), saveFileData);

          final SaveSlot saveSlot =
              slotParser.parse(slotData, loadScreenData, slotDescriptor.getIndex(), slotName);
  ...
  protected boolean isDs1SaveFile(final ByteBlock saveFileData) {
    ...
    final long magicNumber =
        blockSectionParser.parse(saveFileDefinition.getMagicNumberDefinition(), saveFileData);
