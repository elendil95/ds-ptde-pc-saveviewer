...
/**
 * Manager for a {@link SaveWorkspace}: Loads and changes the workspace and the save file and
 * exported save slots in the workspace.
 * 
 * @author illgirni
 *
 */
...
public class WorkspaceManager {
  ...
  /**
   * Loads the save file defined by the managed workspace. The loaded data is stored in the
   * workspace Object.
   * 
  ...
  /**
   * Loads the save slot file defined by the exported slot. The loaded data is stored in the
   * exported slot.
   * 
  ...
  public void loadSaveSlot(final ExportedSlot exportedSlot) throws SaveManagerException {
    final File saveSlotFile =
        new File(getWorkspaceExportedSlotsDirectory(), exportedSlot.getFile());
    final File loadScreenFile =
        new File(getWorkspaceExportedSlotsDirectory(), exportedSlot.getLoadScreenFile());

    exportedSlot.setSlot(
        saveManager.loadSaveSlot(saveSlotFile.getAbsolutePath(), loadScreenFile.getAbsolutePath()));
  }
   ...
   * 
   */
  public void exportSlot(final SaveSlot slotToExport, final String name, final String description,
      final ExportedSlotGroup group) throws SaveManagerException {
    final String slotFileName =
        saveManager.exportSaveSlot(slotToExport, getWorkspaceExportedSlotsDirectory());
    final String loadScreenFileName =
        saveManager.exportSaveSlotLoadScreen(slotToExport, getWorkspaceExportedSlotsDirectory());
  ...
  /**
   * Imports the slot to the given position in the save file of the managed workspace. Reloads the
   * save file before and after conducting the changes.
   * 
   ...
   */
  public void importSlot(final SaveSlot exportedSlot, final int slotIndex)
      throws SaveManagerException {
    loadSaveFile();
  ...
  public void deleteSlot(final ExportedSlot exportedSlot) throws SaveManagerException {
    ...
    saveManager.deleteSaveSlot(getWorkspaceExportedSlotsDirectory(), exportedSlot.getFile(),
        exportedSlot.getLoadScreenFile());
  ...
  /**
   * Creates a new group as sub-group in the given parent group. When no parent group is provided a
   * new root group is created. Saves the workspace after the changes.
   * 
   ...
   * @throws SaveManagerException
   */
  public void createSlotGroup(final String name, final ExportedSlotGroup parentGroup)
      throws SaveManagerException {
  ...
  private void deleteSlots(final ExportedSlotGroup slotGroup) throws SaveManagerException {
    ...
    for (final ExportedSlot slot : slotGroup.getSlots()) {
      saveManager.deleteSaveSlot(getWorkspaceExportedSlotsDirectory(), slot.getFile(),
          slot.getLoadScreenFile());
    }
  }

  /**
   * Moves the group to the new parent. When the new parent is not provided the group will become a
   * root group. Upholds the tree-structure of the groups. Saves the workspace after the changes.
   * 
   ...
   * @throws SaveManagerException
   */
  public void moveSlotGroup(final ExportedSlotGroup movedGroup,
      final ExportedSlotGroup newParentGroup) throws SaveManagerException {
   ...
   * @throws SaveManagerException
   */
  public void moveSlot(final ExportedSlot movedSlot, final ExportedSlotGroup newParentGroup)
      throws SaveManagerException {
   ...
   */
  private boolean containsGroup(final ExportedSlotGroup parentGroup,
      final ExportedSlotGroup descendantGroup) {
   ...
   * @throws SaveManagerException
   */
  private ExportedSlotGroup findParentGroup(final ExportedSlotGroup slotGroup)
      throws SaveManagerException {
    final ExportedSlots exportedSlots = getWorkspace().getExportedSlots();
  ...
  /**
   * Finds the parent of the group starting at the given candidate group. When the candidate group
   * does not contain the group directly descends into the the candidate groups children and their
   * children.
   * 
   * @param slotGroup The group for which to find the parent.
   ...
   */
  private ExportedSlotGroup findParentGroup(final ExportedSlotGroup slotGroup,
      final ExportedSlotGroup candidateGroup) {
   ...
   * @throws SaveManagerException
   */
  private ExportedSlotGroup findParentGroup(final ExportedSlot exportedSlot)
      throws SaveManagerException {
    for (final ExportedSlotGroup candidateGroup : getWorkspace().getExportedSlots()
        .getSlotGroups()) {
  ...
  /**
   * Finds the parent of the slot starting at the given candidate group. When the candidate group
   * does not contain the slot directly descends into the the candidate groups children and their
   * children.
   * 
  ...
  /**
   * Initializes the workspace path. First looks in the system properties for the custom location
   * definition. If defined uses that. if not uses the default location. May create the workspace
   * directory in that case (when it doesn't exist yet).
   * 
   * @throws SaveManagerException
   */
  private void initWorkspacePath() throws SaveManagerException {
    if (System.getProperties()
        .containsKey(WorkspaceConstants.SYSTEM_PROPERTY__CUSTOM_WORKSPACE_LOCATION)) {
      workspacePath = new File(
          System.getProperty(WorkspaceConstants.SYSTEM_PROPERTY__CUSTOM_WORKSPACE_LOCATION));

      if (!workspacePath.isDirectory()) {
        throw new SaveManagerException(
            "Workspace path does not point to a directory: " + workspacePath.getAbsolutePath());
      }

    } else {
      workspacePath = new File(
          System.getProperty(WorkspaceConstants.SYSTEM_PROPERTY__DEFAULT_WORKSPACE_LOCATION),
          WorkspaceConstants.DEFAULT_WORKSPACE_DIRECTORY_NAME);

      if (!workspacePath.isDirectory()) {
        if (workspacePath.isFile()) {
          throw new SaveManagerException("Workspace already exists and is not a directory: "
              + workspacePath.getAbsolutePath());

        } else if (!workspacePath.mkdir()) {
          throw new SaveManagerException(
              "Could not create workspace directory: " + workspacePath.getAbsolutePath());
  ...
  /**
   * Loads the workspace from the workspace file defined by {@link #getWorkspaceFile()}. If the file
  ...
  private void initWorkspace() throws SaveManagerException {
    ...
    if (workspaceFile.isDirectory()) {
      throw new SaveManagerException(
          "Not expecting a directory to be here: " + workspaceFile.getAbsolutePath());

    } else if (!workspaceFile.isFile()) {
      // workspace is empty. setup new one.
      workspace = new SaveWorkspace();
      workspace
          .setSaveFilePath(new File(WorkspaceConstants.DEFAULT_SAVE_FILE_PATH).getAbsolutePath());
  ...
  /**
   * Saves the current workspace state ({@link #getWorkspace()}) to XML at
   * {@link #getWorkspaceFile()}.
   * 
   * @throws SaveManagerException
   */
  ...
  private String getWorkspaceExportedSlotsDirectory() throws SaveManagerException {
    ...
    return new File(workspacePath, WorkspaceConstants.WORKSPACE_EXPORTED_SLOTS_DIRECTORY)
        .getAbsolutePath();
  }
