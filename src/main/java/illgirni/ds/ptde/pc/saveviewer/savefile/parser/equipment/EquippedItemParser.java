...
public class EquippedItemParser extends AbstractSaveElementParser {
  ...
  /**
   * Uses the item definition to parse the corresponding {@link EquippedItem} from the id block and
   * inventory pointer block. Returns {@code null} when the equipment slot is empty.
   * 
   ...
   */
  public EquippedItem parse(final EquippedItemDefinition itemDefinition,
      final ByteBlock itemIdsBlock, final ByteBlock inventoryIndexesBlock) {
    final long itemId =
        blockSectionParser.parse(itemDefinition.getItemIdDefinition(), itemIdsBlock);
    ...
    if (itemId != itemDefinition.getEmptySlotIndicator()) {
      ...
      item.setId(itemId);
      item.setInventoryIndex(blockSectionParser.parse(itemDefinition.getInventoryIndexDefinition(),
          inventoryIndexesBlock));
