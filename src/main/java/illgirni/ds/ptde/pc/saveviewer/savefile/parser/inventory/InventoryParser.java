...
public class InventoryParser extends AbstractSaveElementParser {
    ...
    public Inventory parse(final ByteBlock slotContentData) throws ParserException {
        final ByteBlock inventoryBlock = blockSectionParser.parse(inventoryDefinition.getInventoryDefinition(), slotContentData);
        final List<ByteBlockSectionDefinition<ByteBlock>> itemBlockDefinitions = inventoryDefinition.getInventoryItemDefinitions();
        
        final Inventory inventory = new Inventory();
        
        for (int itemIndex = 0; itemIndex < itemBlockDefinitions.size(); itemIndex++) {
            final ByteBlock itemBlock = blockSectionParser.parse(itemBlockDefinitions.get(itemIndex), inventoryBlock);
