...
public class SaveSlotParser extends AbstractSaveElementParser {
   ...
   * @param loadScreenFilePath The file containing <i>only</i> the data shown on the character
   *        loading screen.
   * @return The parsed save slot.
   * 
   * @throws IOException
   */
  public SaveSlot parse(final String saveSlotFilePath, final String loadScreenFilePath)
      throws IOException {
    ...
    if (saveSlotFile.isFile() && loadScreenFile.isFile()) {
      final ByteBlock saveSlotFileData =
          new ByteBlock(Files.readAllBytes(saveSlotFile.toPath()), 0);
      final ByteBlock loadScreenFileData =
          new ByteBlock(Files.readAllBytes(loadScreenFile.toPath()), 0);

      return parse(saveSlotFileData, loadScreenFileData, -1, null);

    } else if (!saveSlotFile.isFile()) {
      throw new FileNotFoundException(
          "Save slot file not found: " + saveSlotFile.getAbsolutePath());
    } else {
      throw new FileNotFoundException(
          "Load screen file not found: " + loadScreenFile.getAbsolutePath());
    }
  }
   ...
   * @return The parsed save slot.
   */
  public SaveSlot parse(final ByteBlock slotData, final ByteBlock loadScreenData,
      final int slotIndex, final String slotName) {
    final int slotContentLength = blockSectionParser
        .parse(slotDefinition.getSlotContentLengthDefinition(), slotData).intValue();
    final ByteBlock slotContentData =
        blockSectionParser.parse(slotDefinition.getSlotContentDefinition(), slotData);
  ...
  /**
   * Parses the checksum from the byte block representing a save slot and validates the data against
   * the checksum.
   * 
  ...
  protected byte[] parseCheckSum(final ByteBlock slotData) throws CheckSumConflictException {
    final ByteBlockSectionDefinition<byte[]> checkSumDefinition =
        slotDefinition.getCheckSumDefinition();
