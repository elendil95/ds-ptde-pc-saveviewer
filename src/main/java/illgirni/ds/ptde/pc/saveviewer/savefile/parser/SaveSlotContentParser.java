...
/**
 * Parses the character information from a block of bytes representing <i>only</i> the content of a save slot
 * (i.e. excluding the save slots checksum and slot content length definition). Also validates the slot content 
 * against the checksum in the byte block.
 * <p/>
...
public class SaveSlotContentParser extends AbstractSaveElementParser {
    ...
    /**
     * Parses the block of bytes to save slot content. Does not put anything into the slot content,
     * if there is no character name and no character level in the character statistics.
     */
    public SaveSlotContent parse(final ByteBlock slotContentData) throws ParserException {
        ...
        if (characterStats.getName() != null && !characterStats.getName().isEmpty() && characterStats.getLevel() > 0) {
    ...
    protected byte[] parseCheckSum(final ByteBlock slotContentData) throws CheckSumConflictException {
        final ByteBlockSectionDefinition<byte[]> checkSumDefinition = slotContentDefinition.getCheckSumDefinition(slotContentData.getLength());
        final byte[] checkSum = blockSectionParser.parse(checkSumDefinition, slotContentData);
        
        final ByteBlock checkSummedData = ByteUtils.readBlock(slotContentData.getBlockData(), 0, slotContentData.getLength() - checkSum.length);
    ...
    protected void connectEquipmentToInventory(final Equipment equipment, final Inventory inventory) {
        for (final EquippedItem equippedItem : equipment.getEquippedItems()) {
            equippedItem.setInventoryItem(inventory.findItemByInventoryIndex(equippedItem.getIdSpace(), equippedItem.getInventoryIndex()));
        }
        
        for (final AttunedMagic attunedMagic : equipment.getAttunedMagics()) {
            attunedMagic.setInventoryItem(inventory.findItemById(attunedMagic.getIdSpace(), attunedMagic.getId()));
        }
