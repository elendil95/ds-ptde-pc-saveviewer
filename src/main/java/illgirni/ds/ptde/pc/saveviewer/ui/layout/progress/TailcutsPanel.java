...
/**
 * A details panel which shows the tail cuts. For layout reasons they are split in two groups, 
 * which are aligned as a grid with two columns. Each list entry consists of label-value-pairs.
 * The order of the tail cuts corresponds to the discovery order in a "usual playthrough". 
 * 
 * @author illgirni
 *
 */
public class TailcutsPanel extends AbstractDetailsPanel {
    ...
    private int createBlock1(final GridPane tailcutContainer, final int column, int row) {
        List<TailOwner> tailOwnerBlock = Arrays.asList(TailOwner.HELLKITE,
                                                       TailOwner.BELL_GARGOYLES,
                                                       TailOwner.GAPING_DRAGON,
                                                       TailOwner.STONE_DRAGON);
    ...
    private int createBlock2(GridPane tailcutContainer, int column, int row) {
        List<TailOwner> tailOwnerBlock = Arrays.asList(TailOwner.PRISCILLA,
                                                       TailOwner.SEATH,
                                                       TailOwner.SANCTUARY_GUARDIAN,
                                                       TailOwner.KALAMEET);
     ...
     */
    private void createBlock(GridPane tailcutsContainer, int column, int row, List<TailOwner> tailOwnersBlock) {
