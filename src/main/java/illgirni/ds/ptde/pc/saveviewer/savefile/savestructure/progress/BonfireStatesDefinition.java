...
/**
 * Definition for the contents of the bonfire kindle state section in the slot content. The section with the 
 * bonfires moves around in a list of dynamically growing other sections. Also the size of this section is 
 * dynamic. The size probably depends on the areas loaded in the game: Once an area is loaded the bonfires 
 * of that area are added to this section.
 * 
 * @author illgirni
 *
 */
...
public class BonfireStatesDefinition {
    ...
    public ByteBlockSectionDefinition<ByteBlock> getBonfireStateDefinition(final int offset) {
        return new ByteBlockSectionDefinition<>(offset, getBonfireStateBlockLength(), JavaTypeToDataType.BYTE_BLOCK);
    }
