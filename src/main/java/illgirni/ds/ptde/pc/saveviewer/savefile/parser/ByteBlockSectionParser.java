...
/**
 * Parses sections of bytes defined by ByteBlocks. I.e. converts the bytes from a ByteBlock to the
 * type defined by a {@link ByteBlockSectionDefinition}.
 */
@Bean
public class ByteBlockSectionParser {
  ...
  public <T> T parse(final ByteBlockSectionDefinition<T> blockSection, final ByteBlock byteBlock) {
    if (blockSection.getLength() > 0) {
      ...
      return parse(byteBlock.getBlockData(), blockSection.getOffset(), blockSection.getLength(),
          blockSection.getType());
  ...
  /**
   * Parses the value starting at {@code offset} from the block data. For target types with fixed
   * length stops after the number of bytes defined by the target type. For target types with
   * variable length stops parses as much as possible and stops as defined by the type.
   * 
   * @param blockData The data.
   * @param offset Where in the data the value to parse starts.
   * @param type The target type.
