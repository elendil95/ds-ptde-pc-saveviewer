...
/**
 * Panel for a single save slot in the save file. Shows the basic slot information (like i fit is
 * empty, character name, playtime, location, and ng state).
 * 
 * @author illgirni
 *
 */
public class SaveFileSlotPanel extends VBox {
  ...
  public SaveFileSlotPanel(final SaveSlot slot) {
    ...
    focusedProperty()
        .addListener((observable, oldFocused, newFocused) -> applySelectionStyle(newFocused));
    ...
    if (slot.isErroneous()) {
      ...
      columnConstraints.setHalignment(HPos.RIGHT);
      secondRow.getColumnConstraints().addAll(new ColumnConstraints(60), new ColumnConstraints(90),
          columnConstraints);
    }
  ...
  /**
   * Changes the style of the panel according to the current selection style and if the panel is
   * currently focused.
   * 
