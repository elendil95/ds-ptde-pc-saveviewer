...
/**
 * Parses the warp point unlocked information for a single warp point from a "warp point unlocked bit block" 
 * containing information for possibly multiple warp points.
 * 
 * @author illgirni
 *
 */
...
public class WarpPointStateParser extends AbstractSaveElementParser {
     ...
     */
    public WarpPointState parse(WarpPointStateDefinition warpPointStateDefinition, WarpPointGroupBitBlock warpPointGroupBitBlock) {
        final WarpPointState warpPointState = new WarpPointState();
        
        warpPointState.setBonfire(warpPointStateDefinition.getRepresentor());
        warpPointState.setUnlocked(warpPointGroupBitBlock.getBlockData()[warpPointStateDefinition.getBitOffset()]);
