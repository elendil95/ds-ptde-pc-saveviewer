...
public class CheckSumValidator {
  ...
  /**
   * Validates the ByteBlock against the MD5 checksum. Basically calculates the checksum for the
   * byte block and then compares that against the expected checksum.
   * 
   ...
   */
  public void validateCheckSum(final byte[] expectedCheckSum, final ByteBlock checkSummedBlock)
      throws CheckSumConflictException {
    try {
      final byte[] checkSum =
          MessageDigest.getInstance("MD5").digest(checkSummedBlock.getBlockData());
