 ...
 * <p/>
 * The result of the dialog is a pair consisting of a boolean (telling, if a parent has been selected) and
 * the actually selected parent. The actually selected parent can be {@code null} and the boolean 
 * {@code true} when the element is supposed to become a root element.
 * 
 * @author illgirni
 *
 */
public abstract class AbstractMoveExportedElementDialog extends AbstractParentGroupSelectionDialog<Pair<Boolean, ExportedSlotGroup>> {
     ...
     */
    public AbstractMoveExportedElementDialog(final boolean canBeRoot, final ExportedSlotsWrapper parentGroups) {
        ...
        getSelectedParent().addListener((observable, oldValue, newValue) -> okButton.setDisable(newValue == null));
        ...
        setResultConverter(pressedButton -> SAVE_BUTTON_TYPE.equals(pressedButton) ? new Pair<>(getSelectedParent().get() == null, getSelectedParent().get()) : null);
